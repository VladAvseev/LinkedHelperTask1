...
function sostavChisla(massivChisel, chislo) {
    massivChisel = massivChisel.filter((num) => {return num <= chislo});
    const result = [];
    function getCombination(sum, start = 0, arr = []) {
        if (sum === 0 && arr.length > 0 && result.indexOf(arr) === -1) {
            result.push(arr);
        }
        if (start < massivChisel.length) {
            getCombination(sum, start + 1, arr);
            getCombination(sum - massivChisel[start], start + 1, [...arr, massivChisel[start]]);
        }
    }
    getCombination(chislo);
    return result;
}
